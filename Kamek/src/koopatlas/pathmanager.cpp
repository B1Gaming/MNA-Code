...
void dWMPathManager_c::unlockPaths() {
	...
	for (int i = 0; i < pathLayer->pathCount; i++) {
		...
		path->setLayerAlpha((path->isAvailable == dKPPath_s::ALWAYS_AVAILABLE) ? 255 : 102);
	}
	...
	while (*in != 0) {
		...
		for (int i = 0; i < affectedCount; i++) {
			...
			path->setLayerAlpha(value ? 255 : 102);
	...
	for (int i = 0; i < pathLayer->nodeCount; i++) {
		...
		node->setLayerAlpha((node->isUnlocked() & !node->isNew) ? 255 : 102);
	}
}
